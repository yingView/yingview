...
import { Button, Pagination, CheckBoxItem, Utils, Ajax, Dialog } from 'yingview-form';
...
class Email extends Component {
  ...
  queryData(type) {
    ...
    const { userCode } = this.props.location.query;
    Ajax.get({
        ...
        <table style={{ width: '100%' }}>
          ...
          </tr>
          {
            emailList && emailList.length ? emailList.map((item, index) => (
              <tr className="person-email-list" key={index}>
                <td>
                  <CheckBoxItem
                    text={'邮件'}
