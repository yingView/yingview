...
class BookDetail extends Component {
            ...
            <tr key={indx}>
              {
                item && item.map((item, i) => (
                  <Link key={i} to={{ pathname: '/index/articaldetail', query: { articalCode: item.articalCode } }} target={'_blank'}>
        ...
        <ul className="comment-list">
          {
            data && data.map((item, index) => (
              <li className="line clearfix" key={index}>
  ...
  render() {
    const { data, menu, total } = this.state;
    if (!data) {
      return <div />;
    }
    return (
